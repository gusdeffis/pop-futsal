...
// Caso real detectado: el campo del PDF exige coincidencia EXACTA con una
// de sus opciones predefinidas — una diferencia mínima (espacio de más,
// mayúscula distinta, etc., por ejemplo si la hoja de Motivos en Sheets
// quedó levemente distinta al texto del PDF) hacía fallar el .select() en
// silencio, dejando el campo vacío sin ningún aviso. Busca, entre las
// opciones reales del campo, la que coincida ignorando mayúsculas/espacios
// de más — y devuelve ESA (la exacta del PDF), no el texto tal cual vino.
export function encontrarOpcionCoincidente(opciones, valor) {
  const normalizar = (s) => String(s).trim().toLowerCase().replace(/\s+/g, ' ');
  const buscado = normalizar(valor);
  return opciones.find(o => normalizar(o) === buscado);
}
...
// Punto pedido: "Sin balón de back-up" tiene 2 controles INDEPENDIENTES
// (el checkbox de Pantalla4, y "Balón Back-up" de Control Previo en
// Pantalla2 — con sentido invertido: tildado ahí = SÍ hay). Se combinan
// con OR: si CUALQUIERA de los 2 indica el problema, el PDF lo marca.
// Función aparte y pura para poder probarla directo, sin depender del PDF.
export function calcularSinBalonBackup(datos) {
  return !!datos.sin_balon || !datos.balon_backup;
}
...
export async function generarPDFOficial(datos, opciones = {}) {
  ...
  const setChoice = (nombreCampo, valor) => {
    ...
    try {
      ...
      if (c.select) {
        const opciones = c.getOptions ? c.getOptions() : [];
        c.select(encontrarOpcionCoincidente(opciones, texto) || texto);
      } else if (c.setText) {
        c.setText(texto);
      }
  ...
  for (const [claveApp, campoPdf] of Object.entries(CAMPOS_TEXTO)) {
    const valor = claveApp === 'obs_horarios' ? textoHorariosCompleto(datos, { seguido: true }) : datos[claveApp];
  ...
  // Pantalla2 — con sentido invertido: tildado ahí = SÍ hay). Se combinan
  // con OR: si CUALQUIERA de los 2 indica el problema, el PDF lo marca.
  setCheck('sin_balon', calcularSinBalonBackup(datos));
