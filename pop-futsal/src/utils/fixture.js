...
export function parsearFixture(filasCrudas) {
  return (filasCrudas || []).map((fila, i) => {
    ...
    // Solo los campos SECUNDARIOS (no la identidad del partido) se limpian
    // si parecen una fecha rota. Torneo, Local y Visitante NUNCA se tocan
    // acá, aunque se vean raros: un bug real ya pasó por esto — al
    // "limpiarlos" a vacío para mostrarlos mejor, ese vacío después se
    // guardaba de vuelta en la hoja real (porque el guardado reemplaza
    // toda la hoja con lo que hay en memoria), borrando el nombre del club
    // de la planilla de verdad. Mostrar la fecha rota es feo pero
    // recuperable a mano; vaciar la identidad del partido no lo es.
    ['estadio', 'oficial_asignado', 'motivo'].forEach(campo => {
...
export function soloApellido(nombreCompleto) {
