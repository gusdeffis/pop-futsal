...
import CamaraEnPagina from './CamaraEnPagina';
...
// Punto pedido: se saca la foto con la cámara EN PÁGINA (getUserMedia),
// no con la app de Cámara nativa del celular — así el navegador nunca
// pasa a segundo plano, evitando el escenario donde Android mataba la
// pestaña mientras la Cámara nativa estaba abierta. Si el navegador no
// soporta getUserMedia (caso raro), cae al <input type="file"> de
// siempre como plan B.
const soportaCamaraEnPagina = () => typeof navigator !== 'undefined' && !!navigator.mediaDevices?.getUserMedia;
...
function CasilleroFoto({ label, foto, enviada, onSacar, onAbrirCamara, disabled }) {
  const inputRef = useRef(null);
  // Bug real corregido: antes esto se recalculaba en CADA render del
  // componente (por ejemplo, cada vez que se sacaba la OTRA foto y este
  // se volvía a dibujar sin que "foto" haya cambiado) — creando una URL
  // de imagen nueva de más cada vez, con la vieja quedando "colgada" un
  // instante extra antes de liberarse. Con useMemo, solo se crea una URL
  // nueva cuando la foto en sí realmente cambia.
  ...
  const tocar = () => {
    if (disabled) return;
    if (soportaCamaraEnPagina()) onAbrirCamara();
    else inputRef.current?.click(); // plan B: cámara nativa vía input file
  };

  return (
    <div>
      {/* Plan B, oculto — solo se usa si el navegador no soporta getUserMedia */}
      <input
...
export default function FotosFormacionInicial({ partidoId, local, visitante }) {
  ...
  const [camaraAbierta, setCamaraAbierta] = useState(null); // null | 'local' | 'visita'
  ...
  // Bug real detectado y corregido: al principio no había ninguna
  // compresión, y sospechamos que eso causaba el crash de "memoria
  // insuficiente" al sacar una foto — pero probando en el celular real, la
  // versión SIN compresión (guardando la foto tal cual) funcionó bien,
  // mientras que la versión CON compresión (createImageBitmap + canvas)
  // fue la que causaba el crash en ese celular específico — aunque
  // funcionaba perfecto en pruebas de escritorio. Se saca la compresión
  // del medio: se guarda la foto tal cual viene de la cámara, sin procesar.
  //
  // Con la cámara en página (CamaraEnPagina), la foto YA sale liviana
  // (~1MB o menos, ver capturarFotoCamara.js) — así que acá no hace falta
  // comprimir nada más, sea cual sea el origen de la foto (cámara en
  // página o el plan B de <input type="file">).
  const sacarFoto = async (lado, archivo) => {
    setError('');
    try {
  ...
  const capturoEnCamaraEnPagina = (blob) => {
    if (camaraAbierta) sacarFoto(camaraAbierta, blob);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
        <CasilleroFoto label="Foto Local" foto={fotoLocal} enviada={enviadaLocal} onSacar={f => sacarFoto('local', f)} onAbrirCamara={() => setCamaraAbierta('local')} disabled={enviando} />
        <CasilleroFoto label="Foto Visita" foto={fotoVisita} enviada={enviadaVisita} onSacar={f => sacarFoto('visita', f)} onAbrirCamara={() => setCamaraAbierta('visita')} disabled={enviando} />
        <button
      ...
      {camaraAbierta && (
        <CamaraEnPagina onCapturar={capturoEnCamaraEnPagina} onCancelar={() => setCamaraAbierta(null)} />
      )}
    </div>
  );
}
