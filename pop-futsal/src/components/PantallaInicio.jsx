...
// Es el mismo build para popa-prueba.netlify.app (arrastrado a mano, sin
// GitHub) y para producción (GitHub-linked) — en vez de armar 2 builds
// distintos, se fija sola en qué dominio está corriendo. Si en algún
// momento cambia el nombre del sitio de prueba en Netlify, hay que
// actualizar esto.
function esVersionDePrueba() {
  return typeof window !== 'undefined' && window.location.hostname.includes('popa-prueba');
}

export default function PantallaInicio({ guardado, onNuevo, onContinuar, onHistorial, oficiales, pines, oficialLogueado, onLogin, onLogout, esAdmin, esGestion, onAdmin, veInformes, onInformes, veAsignar, onAsignar, pendientes, onCargarPendiente, cargandoPendientes, onActualizarPendientes }) {
      ...
      <div style={{ flex: 1, padding: 24, display: 'flex', flexDirection: 'column', gap: 16, justifyContent: 'center', alignItems: 'stretch', maxWidth: 360, margin: '0 auto', width: '100%', boxSizing: 'border-box' }}>
        ...
        {!oficialLogueado ? (
          <>
            ...
            <div style={{ textAlign: 'center', color: '#000', fontSize: 13, fontWeight: 700 }}>
              {APP_VERSION}{esVersionDePrueba() && ' (prueba)'}
            </div>
          </>
        ) : (
          <>
            ...
            {veAsignar && (
              <button onClick={onAsignar} style={{
                minHeight: 64, background: '#fdf3d8', color: C.rojo, border: `1.5px solid ${C.rojo}`, borderRadius: 10,
                fontSize: 16, fontWeight: 700, cursor: 'pointer', textTransform: 'uppercase', letterSpacing: .3,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
              }}>
                🗓️ Asignar Partidos
            ...
            {esAdmin && (
              <button onClick={onAdmin} style={{
                minHeight: 64, background: '#4a4a48', color: '#fff', border: 'none', borderRadius: 10,
                fontSize: 16, fontWeight: 700, cursor: 'pointer', textTransform: 'uppercase', letterSpacing: .3,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
              }}>
