...
export default function PantallaAsignarPartidos({ onBack, listas, fixtureFilas, clubesFilas: clubesFilasIniciales }) {
  ...
  const [filtros, setFiltros] = useState({ generoMF: '', torneo: '', fechaNro: '', soloPendientes: false, equipo: '', oficial: '', asignacion: '' });
  ...
  const opciones = useMemo(() => ({
    ...
    oficiales: [...new Set((listas?.oficiales || []).map(soloApellido))].filter(Boolean).sort(),
  }), [fixture, listas]);
  ...
  return (
    <div style={{ maxWidth: 480, margin: '0 auto', background: '#fff', minHeight: '100vh', fontFamily: 'system-ui,sans-serif' }}>
      ...
      <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        {!cargando && !error && (
          <>
            {/* Punto 1: una sola línea — M/F, Torneo (angosto), Fecha N°
                (más angosto todavía), y "PEND" a la derecha. */}
            <div style={{ display: 'grid', gridTemplateColumns: '48px 48px 1.3fr 0.5fr 60px', gap: 5 }}>
              {['M', 'F'].map(g => (
              ...
              <button onClick={() => setFiltros(f => ({ ...f, soloPendientes: !f.soloPendientes }))} style={{
                height: 36, borderRadius: 8, fontWeight: 700, fontSize: 11, cursor: 'pointer', padding: '0 4px',
              ...
              {filtrados.map((p, idx) => {
                ...
                return (
                  <div key={filaIndex} style={{
                    ...
                    {!filtros.torneo && (
                      <div style={{ fontSize: 10, color: '#999' }}>{p.torneo} — {p.division === 'M' ? 'Masc.' : 'Fem.'}</div>
                    )}
