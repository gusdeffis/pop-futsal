...
const FILAS_INSTALACIONES = [
  ...
  { cols: 3, items: [
    ['ilum', 'Iluminación', { sinMayuscula: true, tamañoFijo: true }],
  ...
  { cols: 3, items: [['tablero', 'Tablero', { sinMayuscula: true }], ['mesa_crono', 'Mesa Crono', { sinMayuscula: true, lineas: ['Mesa', 'Crono'] }], ['limpieza', 'Limpieza', { sinMayuscula: true }]] },
  { cols: 3, items: [
  ...
  { cols: 3, items: [
    ...
    ['balon_backup', 'Balón Back-up', { sinMayuscula: true, lineas: ['Balón', 'Back-up'] }],
    ['seguridad', 'Seguridad Policía', { sinMayuscula: true, lineas: ['Seguridad', 'Policía'] }],
  ] },
];
// Última fila, aparte del array genérico — Médico pasó a compartir esta
// fila con Camiseta y Partido Suspendido (punto pedido), en el lugar que
// antes quedaba vacío en el medio.
const FILA_MEDICO_CAMISETA = [
  ['medico', 'Médico', { sinMayuscula: true }],
  ['camiseta', 'Camisetas C/Apellido', { sinMayuscula: true, lineas: ['Camisetas', 'C/Apellido'] }],
];
          ...
          <div key={i} style={{ display: 'grid', gridTemplateColumns: `repeat(${fila.cols}, 1fr)`, gap: 8 }}>
            {fila.items.map(([campo, label, opts = {}]) => (
              <CheckAzul
                ...
                minHeight={ALTO_BOTON_INSTALACIONES} padding={PADDING_BOTON_INSTALACIONES} sinMayuscula={opts.sinMayuscula} sinCasillero
                fontSize={opts.tamañoFijo ? 12 : (datos[campo] ? 12 : 13)}
              />
            ))}
          </div>
        ))}

        {/* Última fila: Médico + Camisetas + "Partido Suspendido" — mismo
            tamaño de columna (3) que la fila de Balón/Balón Back-up/
            Seguridad. El botón nuevo funciona distinto a los demás (no es
            un simple casillero de "está en condiciones"): gris cuando no
            está marcado, rojo cuando sí — mismo criterio de color que ya
            usa el botón de Suspensión en la Conclusión final. */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
          {FILA_MEDICO_CAMISETA.map(([campo, label, opts = {}]) => (
            <CheckAzul
              key={campo} label={opts.lineas || label} checked={datos[campo]} onChange={set(campo)}
              minHeight={ALTO_BOTON_INSTALACIONES} padding={PADDING_BOTON_INSTALACIONES} sinMayuscula={opts.sinMayuscula} sinCasillero
              fontSize={datos[campo] ? 12 : 13}
            />
          ))}
          <CheckAzul
