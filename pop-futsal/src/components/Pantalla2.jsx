  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', marginBottom: 10 }}
        ...
        <div style={{ width: 22, height: 22, borderRadius: 4, flexShrink: 0, background: checked ? '#fff' : '#c6dbf5', border: `2px solid ${checked ? '#fff' : '#0d1f4e'}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {checked && <span style={{ color: '#0d1f4e', fontSize: 14, fontWeight: 700 }}>✓</span>}
        </div>
        <span style={{ fontSize: 14, fontWeight: 700, color: checked ? '#fff' : '#0d1f4e' }}>{label}</span>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <span style={{ fontSize: 10, color: checked ? '#ffd0d0' : '#e03030', fontWeight: 700 }}>DEMORA</span>
        <span style={{ fontSize: 12, color: checked ? '#fff' : '#0d1f4e', fontWeight: 600 }}>L</span>
        <input type="number" value={datos[demLKey]} onChange={e => set(demLKey)(e.target.value)}
          placeholder="min" style={{ width: 54, height: 36, border: '1.5px solid #0d1f4e', borderRadius: 6, textAlign: 'center', fontSize: 14, background: '#c6dbf5', color: '#0d1f4e', fontWeight: 600, outline: 'none' }} />
        <span style={{ fontSize: 12, color: checked ? '#fff' : '#0d1f4e', fontWeight: 600 }}>V</span>
...
function CheckVest({ label, campo, datos, set }) {
  ...
  return (
    <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 8, background: checked ? '#0d1f4e' : '#fff', border: '1.5px solid #0d1f4e', borderRadius: 8, padding: '12px 10px', cursor: 'pointer' }}
      onClick={() => set(campo)(!checked)}>
      <div style={{ width: 22, height: 22, borderRadius: 4, background: checked ? '#fff' : '#c6dbf5', border: `2px solid ${checked ? '#fff' : '#0d1f4e'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        {checked && <span style={{ color: '#0d1f4e', fontSize: 14, fontWeight: 700 }}>✓</span>}
      </div>
      <span style={{ fontSize: 12, fontWeight: 700, color: checked ? '#fff' : '#0d1f4e' }}>{label}</span>
    </div>
  );
}
