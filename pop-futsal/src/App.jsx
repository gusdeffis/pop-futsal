...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (guardado && guardado.datos && guardado.datos.torneo) {
  ...
  if (mostrarRecuperar) {
    ...
    return (
      <div style={{ maxWidth: 480, margin: '0 auto', background: '#fff', minHeight: '100vh', fontFamily: 'system-ui,sans-serif', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 32, gap: 16 }}>
        ...
        <div style={{ fontSize: 18, fontWeight: 700, color: '#0d1f4e', textAlign: 'center' }}>
        ...
        <button onClick={recuperar} style={{ width: '100%', height: 52, background: '#0d1f4e', color: '#fff', border: 'none', borderRadius: 8, fontSize: 15, fontWeight: 600, cursor: 'pointer' }}>
          Continuar donde dejé
        </button>
        <button onClick={nuevoPartido} style={{ width: '100%', height: 52, background: '#f8f9fc', color: '#333', border: '1.5px solid #dde1ec', borderRadius: 8, fontSize: 15, cursor: 'pointer' }}>
  ...
  return (
    <div>
      {pantalla === 1 && <Pantalla1 datos={datos} setDatos={setDatos} onNext={() => setPantalla(2)} />}
      {pantalla === 2 && <Pantalla2 datos={datos} setDatos={setDatos} onNext={() => setPantalla(3)} onBack={() => setPantalla(1)} />}
      {pantalla === 3 && <Pantalla3 datos={datos} setDatos={setDatos} onNext={() => setPantalla(4)} onBack={() => setPantalla(2)} />}
      {pantalla === 4 && <Pantalla4 datos={datos} setDatos={setDatos} onBack={() => setPantalla(3)} />}
    </div>
  );
}
