...
export function sheetRowToDatos(p) {
  ...
  const demoraIngreso = (horaIngreso) => {
    if (!g('Hora') || !horaIngreso) return '';
    const [h1, m1] = String(g('Hora')).split(':').map(Number);
  ...
  return {
    ...
    cat: g('Categoría'), dia: g('Día'), hora: g('Hora'), nro: g('Partido N°'),
    ...
    ingreso: g('Ingreso Campo'), protocolo: check('Protocolo Inicio OK'),
    ...
    hora_real: g('Hora Inicio Real'), final_1t: g('Final 1°T'), inicio_2t: g('Inicio 2°T'),
    et_min: g('ET min.'), excedido: check('ET Excedido'), desvio_inicio: '',
    motivo_et: g('Motivo Demora ET'), motivo_et_pdf: g('Motivo Demora ET'),
    final_partido: g('Final Partido'), duracion_partido: '',
    obs_horarios: g('Obs. Horarios'),
    ingreso_local: g('Ingreso Local'), ingreso_visita: g('Ingreso Visita'),
    ingreso_local_dem: demoraIngreso(g('Ingreso Local')), ingreso_visita_dem: demoraIngreso(g('Ingreso Visita')),
